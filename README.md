...
- LLM evaluators can have systematic preferences and blind spots.
- Candidate and evaluator models from the same family may produce correlated judgments; the UI flags this condition.
- Bias and toxicity judgments depend on rubric design and context.
- Faithfulness cannot be fully verified without reference context.
- Scores should be treated as structured diagnostic signals, not absolute truth.
- A serious evaluation study should add human annotations, benchmark datasets, repeated runs, calibration, and inter-rater agreement analysis.

These limitations are intentionally visible because the project is designed to demonstrate critical evaluation thinking, not only API orchestration.
...
2. run `pytest -q`;
3. run the Streamlit app and capture screenshots;
4. inspect `git status`;
5. add screenshots to the README if desired.
...
git init
git add .
git commit -m "Build agentic LLM benchmarking workbench"
git branch -M main
...
# Deploy to Streamlit Community Cloud later
...
- The app can use Streamlit secrets without a `.env` file in the cloud.
- Local `mlflow.db`, `mlruns/`, and `outputs/` storage may be ephemeral on Streamlit Community Cloud.
- The embedded MLflow manifest and downloadable evidence bundle still work for the active session.
- Persistent shared MLflow history would require a remote tracking server and durable artifact storage.

---

# CV description

> Built an agentic LLM benchmarking workbench that separates candidate generation models from specialized evaluator and judge models, enabling blind multi-model comparison across bias, toxicity, faithfulness, reasoning, latency, token usage, and estimated cost. Integrated MLflow parent/child experiment tracking, OpenTelemetry execution tracing, structured JSON evaluation, and interactive Streamlit visualizations.

---

# Possible future improvements

- evaluator calibration against human-labeled benchmark sets;
- repeated judging and uncertainty intervals;
- pairwise comparative judging in addition to absolute scoring;
- prompt-version experiments and evaluator disagreement analysis;
- remote MLflow tracking and OTLP export;
- authentication and per-user benchmark history;
- dataset upload and batch leaderboard generation;
- citation-level verification for RAG responses.
